...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/herotbty/Aiko-XrayR/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/herotbty/Aiko-XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/herotbty/Aiko-XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    file="https://raw.githubusercontent.com/herotbty/Aiko-XrayR/data/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Cài đặt mới, vui lòng tham khảo hướng dẫn trước：https://github.com/herotbty/Aiko-XrayR，Định cấu hình nội dung cần thiết"
    else
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/herotbty/Aiko-XrayR/data/XrayR.sh
